			...
			steps {
				withCredentials([[
						...
						script {							
							...
							def output3= sh(returnStdout: true, script: '/usr/local/bin/aws lexv2-models list-test-execution-result-items --test-execution-id "${output2}" --result-filter-by \'{"resultTypeFilter":"OverallTestResults","conversationLevelTestResultsFilterBy":{"endToEndResult": "Mismatched"}}\' --region "$Region"')
