			...
			steps {
				withCredentials([[
						...
						script {							
							...
							sleep(120)
							def output3= sh(returnStdout: true, script: "/usr/local/bin/aws lexv2-models list-test-execution-result-items --result-filter-by '{\"resultTypeFilter\":\"OverallTestResults\",\"conversationLevelTestResultsFilterBy\":{\"endToEndResult\": \"Mismatched\"}}' --region \"$Region\"  --output text --query \'{testExecutionResults:Differences}\' --test-execution-id ${output2}")
